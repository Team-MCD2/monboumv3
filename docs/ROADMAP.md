...
> ⚠️ **LEGACY** — Since 2026-05-04 the canonical roadmap lives at
> `.project-store/roadmap.md` (M08 schema). This file is kept for
> historical reference of pre-Phase-0 priorities and the Vercel
> readiness ratings (still useful context). For current phase
> tracking, decisions and execution status, defer to:
>
> - `.project-store/roadmap.md` — phases A..F status + DoD
> - `.project-store/decisions.md` — ADR-001..009
> - `docs/STATUS.md` — build / data / SEO health snapshot
>
> Do not edit this file going forward. New entries belong in the
> `.project-store/` files.
